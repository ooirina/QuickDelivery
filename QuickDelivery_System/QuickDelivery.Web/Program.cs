...
var connectionStringIdentity = builder.Configuration.GetConnectionString("IdentityContextConnection")
                               ?? connectionStringApp; // Dacă nu găsește conexiunea ei, o folosește pe a ta
...
// 2. Contextul pentru Identity (Securitate)
...
using (var scope = app.Services.CreateScope())
{
    ...
    try
    {
        // 1. Inițializăm Contextele
        var context = services.GetRequiredService<QuickDeliveryWebContext>();
        var identityContext = services.GetRequiredService<IdentityContext>();

        // Asigurăm crearea tabelelor (Include noile coloane de Latitudine/Logitudine)
        context.Database.EnsureCreated();
        identityContext.Database.EnsureCreated();

        // 2. Gestionare ROLURI (Identity)
        var roleManager = services.GetRequiredService<RoleManager<IdentityRole>>();

        // Folosim .GetAwaiter().GetResult() pentru a evita erorile de compilare asincronă în Program.cs
